...
func TestGetAccountAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		buildStubs    func(store *mock_sql.MockStore)
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer ctrl.Finish()
			store := mock_sql.NewMockStore(ctrl)
			tc.buildStubs(store)
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mock_sql.MockStore) {
				...
				store.EXPECT().
					CreateAccount(gomock.Any(), gomock.Eq(arg)).Times(1).Return(account, nil)
			},
		...
		},
		{
			...
			buildStubs: func(store *mock_sql.MockStore) {
				store.EXPECT().
					CreateAccount(gomock.Any(), gomock.Any()).Times(1).Return(db.Account{}, sql.ErrConnDone)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
			},
